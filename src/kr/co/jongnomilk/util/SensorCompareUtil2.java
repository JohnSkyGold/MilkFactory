...
public class SensorCompareUtil2 {
//	controlCode= buzzer(7,8)| redLed(6) | yellow(5) | green(4) | heat(3) | outlet(2) | entran(1) 
	...
	public static byte getControlCode(SensorData sensorData, PastData pastData) {
	...
	private static void parseCodeAndInsert(byte code, PastData pastData) {
		if(code != 0) {
			...
			if((rightShiftCode(code, 4) & 0b1) == 0b1) {
				...
				if(pastData.getPastYellowLedState() == 1) {
					int rc = 0;
					while(rc == 0) {
						yellowLedLog.setLogData(0);
						rc = logDAO.insertComponentLog(yellowLedLog);
						
					}
					System.out.println("yellow off");
					pastData.setPastYellowLedState(0);
				}
			}	
			//redled
			if((rightShiftCode(code, 5) & 0b1) == 0b1) {
				...
				if(pastData.getPastRedLedState() == 1) {
					int rc = 0;
					while(rc == 0) {
						redLedLog.setLogData(0);
						rc = logDAO.insertComponentLog(redLedLog);
						
					}
					System.out.println("red off");
					pastData.setPastRedLedState(0);
				}
			}
			//buzzer
...
//				}
				...
				SensorData sensorData = new SensorData(f, i, i, j);
